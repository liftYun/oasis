...
const getAddress = async (query: string, signal?: AbortSignal): Promise<AddressSearchResult[]> => {
  try {
    ...
    const body = Array.isArray((data as any)?.body) ? (data as any).body : [];
    return body.filter((d: AddressSearchResult) => Boolean(d?.zone_no));
  } catch (error) {
